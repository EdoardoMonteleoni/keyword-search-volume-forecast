...
Since I usually don't rely solely on these "black-box" tools to gauge a short-term search volume for a keyword set, I will show how I  approach the question using a [data set](search_volume_df.csv) and the program language R to derive a time series linear regression [model](Code.R).
...
One of the advantages of using such a metric is that it is a keyword-based index that considers the number of relevant queries about the advertised product only from interested users.
